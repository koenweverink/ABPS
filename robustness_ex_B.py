...
def main():
    ...
    cfg = {
        ...
        "checkpoint_every": 1,
        "resume_from": None,
        "best_path": "robustness_B.pkl",
        "checkpoint_path": "robustness_B",
        "reeval_best": False,       # re-evaluates the best with num_eval_seeds
        "rng": random.Random(42),           # <-- int seed, not a Random() object
    }
    ...
    with open("robustness_B.json", "w", encoding="utf-8") as f:
        json.dump({"cfg_used": cfg, "result": res}, f, indent=2, default=str)
    with open("robustness_B_summary.json", "w", encoding="utf-8") as f:
